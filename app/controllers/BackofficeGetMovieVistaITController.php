<?php
class BackofficeGetMovieVistaITController extends Controller{ 
	...
	public function movie($id = null){ 	 
		...
		$rowMovie = DB::select("select movieID, movie_Name_EN,
			movie_Img_Thumb, movie_ReleaseDate, movie_Publish, movie_strName, movie_strID
			FROM  movie_list 
			WHERE movieID != 0 
			and movie_statusUpdateData = '0' 
			GROUP BY movie_Name_EN
			ORDER BY movie_Publish asc, movie_ReleaseDate desc, movieID desc"); 
		//** Select End movie_list ****
								 
		return View::make('backend.getMovieVistaIT') -> with('rowMovie', $rowMovie);
	} 
	public function movieEdit($id = null){
		...
		if($movieID != ''){//  Check Edit
			...
			->update(array('movie_Name_CN' => $movie_Name_CN,
			 ...
			 return Redirect::to('backoffice_management/movie#theater');
		}
