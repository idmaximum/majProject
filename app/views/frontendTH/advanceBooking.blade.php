          ...
          <ul id="tab{{$dayTime}}"  <?php if($dayTime != 0){?> class="hide"<?php }?>>
            <li>@foreach($movie_lists as $movie)  
				...
				?><div class="rowShowToday">
                	<div class="rowShowTodayImage"><a href="{{URL::to("th/"."movie/$movie->movieID/$nameMovieSub")}}"><img src="{{asset("uploads/movie/$movie->movie_Img_Thumb")}}" width="140" height="210" class="hoverImg08"></a></div>
                <div class="rowShowTodayDetail">
                  <h2 class="txtGold24 fontTHLang titleMovieName"><a href="{{URL::to("th/"."movie/$movie->movieID/$nameMovieSub")}}">{{$nameMovie}}</a></h2>
            ...
            @foreach($movie_showtimes  as $movieShowtime)
             @if($movie->movie_strID === $movieShowtime->showtime_Movie_strID && $movie->showtime_SystemType == $movieShowtime->showtime_SystemType)
              @if( date('Y-m-d H:i', strtotime($movieShowtime->showtime_dtmDate_Time)) >= $dateNow &&   date('Y-m-d H:i', strtotime($movieShowtime->showtime_dtmDate_Time)) <= $dayTimeNight)<?php
                    ...
                    <div class="showTimeHilight"><span title="Theater 1" class="tooltip theater1Icon"></span></div>@endif 
                    @if($movieShowtime->showtime_strName == 'Theatre 3')
                    <div class="showTimeHilight3"><span title="Theater 3" class="tooltip theater3Icon"></span></div>@endif 
                    <?php 
