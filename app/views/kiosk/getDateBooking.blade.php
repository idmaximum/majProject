    ...
    <ul id="tab{{$dayTime}}" <?php if($dayTime != 0){?>class="hide"<?php }?>>
      <li>@foreach($movie_lists as $movie)
        ...
        <div class="rowShowToday">
          <div class="rowShowTodayImage"><a href="{{URL::to("kiosk/movie/$movieID/$nameMovieSub")}}"><img src="{{asset("uploads/movie/$movie->movie_Img_Thumb")}}" width="140" height="210" class="hoverImg08"></a></div>
          <div class="rowShowTodayDetail">
            <h2 class="txtGold24"><a href="{{URL::to("kiosk/movie/$movieID/$nameMovieSub")}}">{{$movie->movie_Name_EN}}</a></h2>
          ...
          <div class="rowShowTodayShowtime  @if($countShowtime > 6) countShowtime @endif"> 
            ...
            <?php
                    ...
                    <div class="showTimeHilight3"><span title="Hall 3" class="tooltip theater1Icon"></span></div>@endif 
                    <?php 
          ...
          <p class="clear"></p>
      	  ...
      	  @endforeach  
             @if($countMovieShowtime  == '')<div class="noUpdateShowtime">No Updated Showtime</div>@endif
        </li>
    </ul>
    <?php }#end for?>
    </div>
