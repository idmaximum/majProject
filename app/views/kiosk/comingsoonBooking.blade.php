          ...
          <ul id="tab{{$dayTime}}"  <?php if($dayTime != 0){?> class="hide"<?php }?>>
            <li>@foreach($movie_lists as $movie)  
				...
				?><div class="rowShowToday">
                	<div class="rowShowTodayImage"><a href="{{URL::to("kiosk/comingsoon/$movie_comingsoonID/$nameMovieSub")}}"><img src="{{asset("uploads/movie/$movie->movie_Img_Thumb")}}" width="140" height="210" class="hoverImg08"></a></div>
                <div class="rowShowTodayDetail">
                  <h2 class="txtGold24"><a href="{{URL::to("kiosk/comingsoon/$movie_comingsoonID/$nameMovieSub")}}">{{$movie->movie_Name_EN}}</a></h2>
              ...
              @if( date('Y-m-d H:i', strtotime($movieShowtime->showtime_dtmDate_Time)) >= $dateNow &&   date('Y-m-d H:i', strtotime($movieShowtime->showtime_dtmDate_Time)) <= $dayTimeNight)
                  <?php
                    ...
                    <?php 
					  ...
					  ?>
                   <a href="{{URL::to("$urlPage")}}" title="Audio: {{subLanguage($audio)}}   / Subtitle: {{subLanguage($subtitle)}}" >{{ date('H:i', strtotime($movieShowtime->showtime_dtmDate_Time))}}</a>	
                   <?php
