...
<ul class="breadcrumbs">
  ...
  <li>Movie List Edit</li>
</ul>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="70%"><h1 class="pagetitle">Movie List Edit</h1></td>
...
<div class="detail-page"> @foreach ($rowMovie as $row)
  <form method="post" enctype="multipart/form-data" name="movieEdit" class="form-horizontal form-page formular"id="newsForm" role="form">
    ...
    <div class="form-group">
      ...
      <div class="col-lg-9">
        <input type="text" class="form-control" name="movie_Name_CN"  id="title_cn" placeholder="หัวข้อ (CN)" value="{{$row->movie_Name_CN}}">
      </div>
    </div>
    <div class="form-group">
      ...
      <div class="col-lg-9">
        ...
        <div class="pic-img"> @if($row->movie_Img_Thumb != '') <img id="targetEdit" src="{{asset('uploads/movie')}}/{{$row->movie_Img_Thumb}}" alt="your image"style="max-width:250px;  max-height:250px"/> @else
          ไม่มีรูปประกอบ
          @endif </div>
      </div>
    </div>
    ...
    <div class="form-group">
      <label for="news_detail" class="col-lg-3 control-label">เนื้อเรื่องย่อ (CN) </label>
      <div class="col-lg-9">
        <textarea name="movie_Synopsis_CN" class="form-control"  cols="90" rows="10" id="detail_cn" >{{$row->movie_Synopsis_CN}}</textarea>
      </div>
    </div>
    <div class="form-group">
      ...
      <div class="col-lg-9">
        ...
        <select name="movie_Rating" class="form-control">
          <option value="ส." @if($movie_Rating == 'ส.') selected="selected"@endif>(ส.) = ส่งเสริม</option>
          <option value="ทป." @if($movie_Rating == 'ทป.') selected="selected"@endif>(ทป.) = ทั่วไป</option>
          <option value="น13+" @if($movie_Rating == 'น13+') selected="selected"@endif>(น13+) = เหมาะสำหรับผู้ที่มีอายุ 13 ปีขึ้นไป</option>
          <option value="น15+" @if($movie_Rating == 'น15+') selected="selected"@endif>(น15+) = เหมาะสำหรับผู้ที่มีอายุ 15 ปีขึ้นไป</option>
          <option value="น18+" @if($movie_Rating == 'น18+') selected="selected"@endif>(น18+) = เหมาะสำหรับผู้ที่มีอายุ 18 ปีขึ้นไป</option>
          <option value="ฉ20-" @if($movie_Rating == 'ฉ20-') selected="selected"@endif>(ฉ20-) = ห้ามผู้ที่มีอายุต่ำกว่า 20 ปี เข้าชม</option>
        </select>
    ...
    <div class="form-group">
      ...
      <div class="col-lg-9">
        ...
        <br />
        Ex. http://www.youtube.com/watch?v=G3Rzy7YqUVU </div>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
      ...
      <div class="col-lg-9">
        <input  name="movie_ReleaseDate" type="text" class="form-control datepicker" id="movie_ReleaseDate" placeholder="Release Date" value="{{date("Y-m-d", strtotime($row->
        movie_ReleaseDate)) }}"> 
        EX.2014-04-24 </div>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
      <div class="col-lg-offset-3 col-lg-9">
        ...
        <input name="movie_strID" type="hidden" id="movie_strID" value="{{$row->movie_strID}}" />
      </div>
    </div>
  </form>
  @endforeach </div>
