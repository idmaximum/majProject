  ...
  <div id="content" class="content-main"> 
    ...
    <div class="contentPage">
	  ...
	  ?>
     	...
     	<div class="divImgNews"><a href="{{URL::to('event_activity/'.$row->news_ID.'/'.$nameNewsSub)}}" title="{{$newsName}}"><img src="{{asset('uploads/news/'.$row->news_imageThumb)}}" style="max-width:300px" class="hoverImg08"></a></div>
        <div class="divDetailNews">
       	  <h2 class="txtWhite20"><a href="{{URL::to('event_activity/'.$row->news_ID.'/'.$nameNewsSub)}}" title="{{$newsName}}">{{$newsName}}</a></h2>
             <p class="timeShow">{{ date('d F Y', strtotime($row->news_datetime))}}</p>
          <p class="txtBrown18_2">{{$row->news_abstract_en}}</p>
        </div>
   	  </div>
  	    <?php $numRow++?>
         @endforeach
