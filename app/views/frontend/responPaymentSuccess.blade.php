<?php
	...
	$paymentResult = '';
	$paramsGetPrintStreamRequest = '';
	$GetPrintStreamResponse = '';
 ...
 $wsdl = "http://10.121.130.25/WSVistaWebClient/TicketingService.asmx?WSDL"; 
	...
	$CompleteOrderResponse = json_encode($completeOrderData); 
		...
		//*******************
		if($paymentResult == 'UnexpectedError'){	
			  $wsdlGetPrintStream  = "http://10.121.130.25/WSVistaWebClient/DataService.asmx?WSDL";  
			  $clientGetPrintStream  = new SoapClient($wsdlGetPrintStream, array('trace' => true)); 
			  
			  $paramsGetPrintStream = array(	
								  'UserSessionId'							=>		$userSessionID,
								  'PrintDocumentType'					=> 		"All",
								  'PrintDocumentCode'					=> 	 0
								  );
			  
			$resultGetPrintStream = $clientGetPrintStream->__soapCall('GetPrintStream', array($paramsGetPrintStream)); 
			$paramsGetPrintStreamData = json_decode(json_encode($resultGetPrintStream), true);	
			   
			$paramsGetPrintStreamRequest = json_encode($paramsGetPrintStream);
			$GetPrintStreamResponse 		= json_encode($paramsGetPrintStreamData); 
			
			  $pincodeXML = $resultGetPrintStream->DatasetXML;   
					 
			  $xmlPincode = simplexml_load_string($pincodeXML);
			  $jsonPincode = json_encode($xmlPincode); 
			  $resultPincode = json_decode($jsonPincode,TRUE);
			  $pincode = $resultPincode["Table"]['OrderP_intVistaBookingNumber'];
   } #end  
	...
	$SOAPprocess = $dataTblLog.', CompleteOrder';
 	 ...
 	 DB::table($logTable)
			...
			->update(array('soap_process'			=> $SOAPprocess,  
						   ...
						   'getprintstream_request'	=> $paramsGetPrintStreamRequest ,
						   'getprintstream_response'	=> $GetPrintStreamResponse  			 
				));   
...
<body> 
<div id="main">
 @foreach($log_DataDetail as $logdetail)
 <?php
 	$SeatPrice 		=	'888'; 
   $seatPosition 	=	''; 
   $cname 			=	''; 
   $mobile			=	'';
	...
	$SeatPrice 		=	$logdetail->amount; 
   $seatPosition 	=	$logdetail->seat; 
   $cname 			=	$logdetail->cname; 
   $mobile			=	$logdetail->phone;  
 ...
 @endforeach
  @include('frontend.incHeader')  
      ...
      <p style="text-align:center"><img src="{{asset("images/ic_confirmation.png")}}" width="252" height="65"></p>
      ...
      <div class="content-detail-movie">
          ...
          <table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			?><p><img src="{{asset("images/icon/$imgRate")}}" alt="{{$movie->movie_Rating}}" style="padding:0; max-width:40px"> 
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="detailComplate">
                  <tr>
                    <td width="13%">Date </td>
                    <td width="21%" class="txtWhite18">{{ date('d F Y', strtotime($movieSession)) }}</td>
                    <td width="15%">Hall</td>
                    <td width="21%" class="txtWhite18">{{strReplaceText($movie->showtime_strName)}}</td>
        ...
        <div class="content-seat-type"> 
          ...
          <?php	$pincodeQR = 	sprintf("%07d",$pincode); ?>     	
          	<p><img src="https://chart.googleapis.com/chart?chs=250x250&amp;cht=qr&amp;chl={{$pincodeQR}}&amp;choe=UTF-8" alt="QR code"></p>
          	<p style="padding-top:5px;letter-spacing:2px ; font-size:22px" class="txtWhite22"><span style="letter-spacing:0px">Booking No. : </span> {{$pincode}}</p>
          </div>
          <p>&nbsp;</p>
          <p class="txtRed18" style="padding:10px 0 12px 0;">Please pick up your tickets at the Box Office.</p>
          <p class="clear"></p>
        </div>
      </div> 
    </div> 
...
Mail::send('emails.enewsPayment', $data , function($message)use ($user){  
	  $message->to($user['email'], $user['email'])->subject('Movie ticket Payment notification from Embassycineplex.com');
 }); /**/
